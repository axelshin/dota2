...
var myo = (function(window, document, undefined) {
    ...
    function showDim () {
        ...
        dimDiv.css("display", "block").stop().animate({opacity: 0.7}, 100);
    }
    function hideDim () {
        if (!dimDiv.length) return false;
...
(function() {
    function Fl_popup(){
        ...
        var circleTrigger = '.circle--trigger-js',
            circleElem = '.circle--js',
            circleClass = 'fl-popup';
        
        if (!$(circleTrigger).length) {
            return
        }
        myo.open({
            clas: circleClass,
            elem: circleElem,
            beforeOpen: function(){
                $(circleTrigger).addClass('circle--hidden');
            },
            afterClose: function(){
                $(circleTrigger).removeClass('circle--hidden');
            },
        });        
    };

    $(document).on('click', '.circle--trigger-js', function(e){
        e.preventDefault();
        Fl_popup();
    })

    $(document).on('click', '.circle__link--js', function(e){        
        myo.close();
    })

    setTimeout(Fl_popup, 3500)
